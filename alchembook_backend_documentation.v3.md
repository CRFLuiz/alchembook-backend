# Documentação do Backend Alchembook - v3
...
**Além disso, foi adicionada uma camada de persistência de dados com PostgreSQL e Sequelize, permitindo o gerenciamento de usuários e outras informações estruturadas.**
...
### Persistência de Dados (PostgreSQL e Sequelize)

*   **Sequelize**: ORM (Object-Relational Mapper) utilizado para interagir com o banco de dados PostgreSQL.
*   **Modelos**: Definem a estrutura das tabelas no banco de dados e as interações com os dados.
    *   **`User`**: Modelo para gerenciar informações de usuários (cadastro e login). Contém campos como `username`, `email` e `password`.
*   **Migrations**: Arquivos que descrevem as alterações no esquema do banco de dados (criação de tabelas, adição de colunas, etc.). São versionadas e permitem controlar a evolução do banco de dados.
*   **Seeders**: Arquivos para popular o banco de dados com dados iniciais ou de teste.
...
*   **ORM**: **Sequelize**
*   **Banco de Dados**: **PostgreSQL** (com drivers `pg` e `pg-hstore`)
*   **Ferramenta de Linha de Comando para ORM**: **`sequelize-cli`**
...
*   **Principais Funções/Lógica**:
    ...
    *   **Configuração do Redis Client**:
        ...
        *   `redisClient.on('connect', ...)` e `redisClient.on('error', ...)`: Listeners para eventos de conexão do Redis.
...
*   **`config/config.json`**: Arquivo de configuração do Sequelize para conexão com o banco de dados, incluindo configurações para ambientes de desenvolvimento, teste e produção.
*   **`models/`**: Diretório que contém as definições dos modelos do Sequelize, como `user.js`.
*   **`migrations/`**: Diretório que contém os arquivos de migração do banco de dados, como `[timestamp]-create-user.js`.
*   **`seeders/`**: Diretório que conterá os arquivos de seeder para popular o banco de dados.
